...
const getDashboard = async (req, res) => {
  try {
    ...
    const [[{ total_siswa }]] = await db.query(
      "SELECT COUNT(*) AS total_siswa FROM students WHERE user_id = ?",
      [req.user.id],
    );

    // Ringkasan risiko berdasarkan prediksi terbaru tiap siswa
    const [risikoRows] = await db.query(
      `SELECT p.risk_category, COUNT(*) AS jumlah
   FROM students s
   ...
   WHERE p.risk_category IS NOT NULL AND s.user_id = ?
   GROUP BY p.risk_category`,
    ...
    const [siswaBerisiko] = await db.query(
     ...
     p.risk_category, p.confidence, p.risk_factors,
   ...
   WHERE p.risk_category IN ('High', 'Medium') AND s.user_id = ?
   ORDER BY
    ...
    res.status(200).json({
      ...
      data: {
        ringkasan: {
          ...
          belum_diprediksi:
            total_siswa -
            (ringkasanRisiko.High +
              ringkasanRisiko.Medium +
              ringkasanRisiko.Low),
        },
...
const markNotifRead = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Notifikasi ditandai sudah dibaca." });
  } catch (err) {
