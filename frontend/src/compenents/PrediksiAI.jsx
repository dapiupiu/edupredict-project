...
function PrediksiAI({ predictionResult, hideHeader = false, hideDistribusi = false }) {
  ...
  const risk_factors = typeof prediksi.risk_factors === 'string' 
    ? JSON.parse(prediksi.risk_factors) : (prediksi.risk_factors || []);
  ...
  }) : [{ title: "Tidak ada faktor risiko dominan terdeteksi.", value: "", color: "text-green-700", dot: "bg-green-600" }];
            ...
            {formattedFactors.map((factor, index) => (
              ...
              >
                <div className="flex items-center gap-3 mb-2">
                  <div
                    className={`w-3 h-3 rounded-full ${factor.dot}`}
                  ></div>
